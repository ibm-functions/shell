...
describe('Create actions, switch to parameters view, then add parameters', function() {
    ...
    it('should switch back to code mode', () => cli.do('code', this.app)
       ...
       .then(sidecar.expectShowing('foo'))
       .then(app => app.client.getText(`${ui.selectors.SIDECAR_CONTENT} .action-source`))
       .then(text => assert.equal(text.replace(/\s+/g,''), 'functionmain(params){return{name:"Step1"+params.name};}')))
})
