...
const updateMapping = (op, attr, key, value) => entity => {
    ...
    if (!toplevel) {
        ...
        if (op === 'set') {
            ...
            if (!util.isArray(toplevel.value)) {
        ...
        if (leaf) {
            if (op === 'set') {
                ...
                if (!leaf.parent[leaf.key]) {
                    ...
                    leaf.parent[leaf.key] = [value]
                } else if (!util.isArray(leaf.parent[leaf.key])) {
 ...
 *
 */
const add = (wsk, type) => (op, opKind=op, attr='parameters') => (block, nextBlock, _1, modules, rawCommand, _3, _4, options) => {
    /** fetch the given entity with the given type */
    const fetchEntityWithType = (name, type) => modules.repl.qexec(`wsk ${type} get ${name}`)
    ...
    {
        let key, value, dest, tryThisType
        const selection = modules.ui.currentSelection()

        const command = rawCommand.substring(rawCommand.indexOf(op))

        const matcher1 = patterns[opKind].implicitEntity,
              matcher2 = patterns[opKind].explicitEntity,
              match1 = command.match(matcher1.pattern),
              match2 = command.match(matcher2.pattern)
        ...
        return fetchEntity(dest, tryThisType)                                                // grab a copy of the current attributes
            ...
            .then(entity => modules.ui.showEntity(entity, { show: attr }, block, nextBlock)) // open the entity, showing the attribute, e.g. parameters or annotations
            .then(() => true)
    }
}
